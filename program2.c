...
{
	file_2_sudoku(sudoku,"test_1.txt");
	print_sudoku(sudoku);
	solve_sudoku(sudoku);
...
int file_2_sudoku(int *sudoku, char *filename)
{
	...
	while ((c != EOF)){
		c = fgetc(fp);
		if ((((c - 48) < 0) || ((c - 48) > 9)) && (c != EOF)) // ASCII to int conversion
			error=1;
		*sudoku ++= (c - 48);
		i++;
    }
...
int solve_sudoku(int *problem)
{
	...
	while(check_all(problem) != 0){
		(*ptr)++;
		if ((*ptr) == 10){
			(*ptr) = 0;	// If there is not a valid number, we write a 0 and return an error
			return 1;
		}
    }
	...
	return 0; // If we reach this instruction, all is good.
}
